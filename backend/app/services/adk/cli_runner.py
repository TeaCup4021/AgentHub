...
_DEFAULT_ISOLATED_WORKSPACE = os.path.join(os.path.expanduser("~"), ".agenthub", "cli_workspace")
...
def _resolve_cli_workspace(workspace_dir: str) -> str:
    """Keep CLI outputs away from the backend reload watcher."""
    requested = os.path.abspath(workspace_dir or _DEFAULT_ISOLATED_WORKSPACE)
    unsafe_roots = (_PROJECT_ROOT, _BACKEND_ROOT)
    if any(_is_same_or_parent(root, requested) for root in unsafe_roots):
        logger.warning(
            "CLI workspace %s overlaps the AgentHub source tree; using isolated workspace %s",
            requested,
            _DEFAULT_ISOLATED_WORKSPACE,
        )
        return _DEFAULT_ISOLATED_WORKSPACE
    return requested
...
class BaseCliRunner:
    ...
    def __init__(self, cli_path: str, workspace_dir: str = ".") -> None:
        ...
        self._workspace_dir = _resolve_cli_workspace(workspace_dir)
...
class ClaudeCodeRunner(BaseCliRunner):
    def _build_args(self, prompt: str, **kwargs: Any) -> list[str]:
        ...
        return [
            ...
            "--strict-mcp-config",  # Disable MCP servers to prevent context pollution
            "--disable-slash-commands",  # Disable skills to prevent context pollution
        ]
    ...
    def _parse_line(self, line: str) -> list[CliEvent]:
        ...
        if event_type == "system":
            ...
            # With --include-partial-messages, Claude Code emits BOTH:
            #   (a) stream_event → content_block_delta (incremental text)
            #   (b) assistant → full message content (text + thinking + tool_use)
            # Emitting text from BOTH duplicates every token in the accumulated content.
            # We skip text parts here — they already arrive via stream_event deltas;
            # if no deltas arrived for some reason, the "result" event serves as
            # fallback. Non-text parts (thinking, tool_use) only come via assistant,
            # so we keep those.
            ...
            for part in message.get("content", []):
                if isinstance(part, dict):
                    ...
                    if part_type == "text":
                        # Skip — already received via stream_event content_block_delta
                        pass
                    ...
                    # Legacy plain-string parts — keep for safety (rare)
