...
<artifact type="file" url="DOWNLOAD_URL" name="filename.ext" size="BYTES" mime="mime/type" />

5. If a user asks for a PDF download link, call create_file with a .pdf file_path.
The create_file tool can create real PDF files; do not substitute HTML unless the
...
_ARTIFACT_REMINDER = (
    "\n\n[SYSTEM] Wrap any code, diffs, files, or HTML pages you create in "
    "<artifact type=\"...\"> XML tags. Use CDATA sections for code/HTML bodies. "
    "For PDF download requests, use create_file with a .pdf file_path and return "
    "the generated download URL as a file/document artifact; do not offer HTML as "
    "a PDF substitute unless explicitly requested. [/SYSTEM]"
)
