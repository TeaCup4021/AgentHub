...
class MessageService:
    ...
    async def persist_stream_message(
        ...
        if existing:
            return existing
        parsed_sender_id = None
        if sender_id:
            try:
                parsed_sender_id = UUID(str(sender_id))
    ...
    async def list_messages(
        ...
        if any(m.sender_type == "agent" and not m.sender_id for m in messages):
            ...
            participant_ids = list(dict.fromkeys(parts_result.scalars().all()))
            if len(participant_ids) == 1:
                fallback_agent = await db.get(Agent, participant_ids[0])
                if fallback_agent:
                    fallback_agent_id = fallback_agent.id
                    fallback_agent_name = fallback_agent.name

        def _resolve_name(m: Message) -> str:
            # Per-message override: DAG/group agent messages have sender_id=None
            # and carry their real agent name in meta_data.agent_name. Resolving
            # via (sender_type, sender_id) alone would collapse all such messages
            # onto a single name (the last writer wins), so prefer the message's
            # own meta_data first.
            if m.sender_type == "agent" and not m.sender_id and fallback_agent_name:
                return fallback_agent_name
            if m.sender_type == "agent" and isinstance(m.meta_data, dict):
                name = m.meta_data.get("agent_name")
                if name:
                    return name
            return sender_names.get((m.sender_type, m.sender_id), "Unknown")

        items = [
            MessageService._format_message(
                m,
                _resolve_name(m),
                art_map.get(m.id, []),
                ...
                sender_id_override=(
                    fallback_agent_id
                    if m.sender_type == "agent" and not m.sender_id and fallback_agent_id
                    else None
                ),
            )
            for m in messages
    ...
    def _format_message(
        ...
        sender_id_override: UUID | None = None,
    ) -> dict:
        return {
            "id": msg.id,
            "conversation_id": msg.conversation_id,
            "sender_type": msg.sender_type,
            "sender_id": sender_id_override or msg.sender_id,
