...
class TestRichMediaArtifactSupport:
    ...
    async def test_tool_download_response_artifact(self):
        function_response = SimpleNamespace(
            response={
                "result": (
                    'Successfully created file sample.pdf\n'
                    '{"download_url": "/api/v1/files/44444444-4444-4444-4444-444444444444/download", '
                    '"file_name": "sample.pdf", "file_size": 512, "mime_type": "application/pdf"}'
                )
            }
        )
        event = create_event()
        event.get_function_responses = lambda: [function_response]

        result = await self.process_single_event(event)

        assert result is not None
        assert result["artifact"]["artifactType"] == "document"
        assert result["artifact"]["content"]["fileName"] == "sample.pdf"
        assert result["artifact"]["content"]["fileType"] == "pdf"
