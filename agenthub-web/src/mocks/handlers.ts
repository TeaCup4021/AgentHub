...
function generatePlan(agentIds: string[]): PlanSubtask[] {
  const stageCount = Math.max(2, Math.min(4, agentIds.length || 2));
  const stageIds = Array.from({ length: stageCount }, () => `sub-${generateId()}`);
  return stageIds.map((stageId, i) => ({
    subtask_id: stageId,
    instruction: i === 0
      ? "Clarify requirements and define the execution approach"
      : i === stageCount - 1
        ? "Review results and prepare the final summary"
        : `Implement stage ${i}`,
    recommended_capabilities: i === stageCount - 1 ? ["review", "summary"] : ["implementation"],
    acceptance_criteria: ["The stage output is concrete and ready for orchestration"],
    can_parallel: i > 0 && i < stageCount - 1,
    depends_on: i === 0 ? [] : [stageIds[i - 1]],
    priority: i + 1,
  }));
}
  ...
  const interceptor = api.interceptors.request.use(async (config) => {
    ...
    // POST /conversations/:id/messages
      ...
      const body = parseBody(config) as unknown as {
        ...
        mode?: "auto_orchestrate" | "direct" | "refine_plan" | "confirm_plan" | "agent_builder";
        plan_id?: string;
        plan?: PlanSubtask[];
