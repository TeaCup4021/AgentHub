...
// npmmirror 正确格式：registry.npmmirror.com/包名/版本/files/路径
loader.config({ paths: { vs: "https://registry.npmmirror.com/monaco-editor/0.55.1/files/min/vs" } });
...
export function MonacoCodeEditor({ code, language, fileName, readOnly = false, onChange, onSave }: MonacoCodeEditorProps) {
  ...
  return (
    <div style={{ border: "1px solid var(--color-border-light)", borderRadius: "var(--radius-sm)", overflow: "hidden" }}>
      ...
      <Editor
        height={Math.max(200, Math.min(600, localCode.split("\n").length * 20 + 40))}
        language={language || "text"}
        value={localCode}
        theme={resolvedTheme}
        onChange={(v) => { setLocalCode(v ?? ""); onChange?.(v); }}
        options={{
          readOnly, minimap: { enabled: false },
