...
const EMPTY_ASSISTANT_MESSAGE = "Agent response was interrupted before producing output. Please retry.";
...
export function getUserBubbleContent(content: string): string {
  const text = content.trim();
  if (!text.startsWith("[选区修改]")) return content;

  const match = text.match(/(?:^|\n)修改要求：([\s\S]*)$/);
  const request = match?.[1]?.trim();
  return request || "修改选中代码片段";
}
...
function getPlanMeta(message: Message): {
  planId: string;
...
function isBackendStaticLink(url: string): boolean {
  try {
    ...
    return path === "/" || path === "" || path.endsWith(".html") || path.endsWith(".htm");
  } catch {
    return false;
  }
}
...
function renderFallbackCards(
  ...
  convId?: string,
  options: { includeLinkPreview?: boolean } = {},
) {
  const hasArtifactType = (type: string) => existingArtifacts.some((a) => a.artifactType === type);
  const cards: React.ReactNode[] = [];
  const includeLinkPreview = options.includeLinkPreview ?? true;
  ...
  if (includeLinkPreview && !hasArtifactType("link_preview")) {
    ...
    while ((match = urlRe.exec(content)) !== null) {
      ...
      if (isBackendStaticLink(url)) continue;
...
const MessageBubble = memo(function MessageBubble({ message, agents, searchText, onRegenerate, onConfirmPlan, onAdjustPlan, onRefinePlan, dagTaskId, onPin, onUnpin }: {
  ...
  const displayContent = isUser
    ? redactSensitiveText(getUserBubbleContent(cleanContent(message)))
    : cleanContent(message);
  const planMeta = isOrchestrator || message.contentType === "plan" ? getPlanMeta(message) : null;
  ...
  const renderableArtifacts = getRenderableArtifacts(message.artifacts);
  const hasArtifacts = renderableArtifacts.length > 0;
  const hasVisibleBody =
    Boolean(displayContent.trim()) ||
    thinkingSteps.length > 0 ||
    hasAttachments ||
    hasArtifacts;
  const shouldShowEmptyFallback = !planMeta && !isUser && !hasVisibleBody;
            ...
            <ErrorBoundary label="消息渲染">
              {planMeta ? (
                ...
                <>
                  ...
                  {displayContent.trim() && (
                    <MarkdownBubble
                      text={searchText ? highlightText(displayContent, searchText) : displayContent}
                    />
                  )}
                  ...
                  {renderableArtifacts.map((a) => <CardRenderer key={a.id} artifact={a} convId={message.conversationId} />)}
                  {renderFallbackCards(
                    displayContent,
                    renderableArtifacts,
                    message.conversationId,
                    { includeLinkPreview: !isUser },
                  )}
                </>
              )}
            </ErrorBoundary>
...
const StreamingMessageBubble = memo(function StreamingMessageBubble({ messageId, agentName, convId }: { messageId: string; agentName: string; convId?: string }) {
  ...
  if (!sc) return null;
  const renderableArtifacts = getRenderableArtifacts(sc.artifacts);
  const hasStreamOutput =
    stripArtifactTags(sc.content).trim().length > 0 ||
    renderableArtifacts.length > 0 ||
    sc.thinkingSteps.length > 0;
  return (
      ...
      <div style={{ maxWidth: "75%" }}>
        ...
        <div style={{
          ...
          <ErrorBoundary label="流式消息渲染">
            ...
            {sc.content && <MarkdownBubble text={redactSensitiveText(stripArtifactTags(sc.content))} isStreaming />}
            {!hasStreamOutput && (
              <div
                data-testid="streaming-output-loading-bar"
                className="streaming-output-loading"
                aria-label="正在生成"
              >
                <span />
              </div>
            )}
            {renderableArtifacts.map((a) => <CardRenderer key={a.id} artifact={a} convId={convId} />)}
            {renderFallbackCards(redactSensitiveText(sc.content), renderableArtifacts, convId)}
          </ErrorBoundary>
        </div>
      </div>
    </div>
