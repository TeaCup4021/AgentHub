...
export const MessageActions = memo(function MessageActions({ message, isStreaming, isFailed, onRegenerate }: MessageActionsProps) {
  ...
  const handleCopy = useCallback(() => {
    navigator.clipboard.writeText(redactSensitiveText(message.content)).then(() => {
  ...
  const handleQuote = useCallback(() => {
    setPendingQuote({ messageId: message.id, content: redactSensitiveText(message.content) });
