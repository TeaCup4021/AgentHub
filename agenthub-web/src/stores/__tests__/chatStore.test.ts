...
describe("chatStore — appendStreamArtifact", () => {
  ...
  it("相同 id 的 artifact 应原地覆盖，避免重复卡片", () => {
    useChatStore.getState().initStreamingMessage("msg-1");
    useChatStore.getState().appendStreamArtifact("msg-1", {
      id: "download-1", artifactType: "document" as const, title: "old.pdf",
      content: { fileName: "old.pdf", fileUrl: "/old", fileType: "pdf", fileSize: 1 },
      version: 1, createdAt: "2026-01-01T00:00:00Z",
    });
    useChatStore.getState().appendStreamArtifact("msg-1", {
      id: "download-1", artifactType: "document" as const, title: "new.pdf",
      content: { fileName: "new.pdf", fileUrl: "/new", fileType: "pdf", fileSize: 2 },
      version: 1, createdAt: "2026-01-01T00:00:00Z",
    });

    const artifacts = useChatStore.getState().streamingContent["msg-1"].artifacts;
    expect(artifacts).toHaveLength(1);
    expect(artifacts[0].title).toBe("new.pdf");
  });
});
