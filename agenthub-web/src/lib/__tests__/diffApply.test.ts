...
import { applySnippet, findApplyTarget, resolveDiffDisplay, type CodeCandidate } from "../diffApply";
...
describe("resolveDiffDisplay", () => {
  it("expands a snippet diff to full before/after code for display", () => {
    const r = resolveDiffDisplay(
      {
        fileName: "quicksort.py",
        oldCode: "    pivot = arr[len(arr) // 2]",
        newCode: "    pivot = arr[0]",
    ...
    expect(r.oldCode).toBe(QS);
    expect(r.newCode).toContain("pivot = arr[0]");
    expect(r.newCode).toContain("return quicksort(left) + middle + quicksort(right)");
  });

  it("reconstructs full before/after display after the snippet has already been applied", () => {
    const applied = QS.replace("    pivot = arr[len(arr) // 2]", "    pivot = arr[0]");
    const r = resolveDiffDisplay(
      {
        fileName: "quicksort.py",
        oldCode: "    pivot = arr[len(arr) // 2]",
      ...
      [makeCandidate({ code: applied })],
    );

    expect(r.expanded).toBe(true);
    expect(r.matchType).toBe("reverse-snippet");
    expect(r.oldCode).toBe(QS);
    expect(r.newCode).toBe(applied);
  });

  it("falls back to the raw snippet when no source code card matches", () => {
    const r = resolveDiffDisplay(
      { fileName: "other.py", oldCode: "bubble_sort", newCode: "maopao" },
      [makeCandidate()],
    );

    expect(r.expanded).toBe(false);
    expect(r.oldCode).toBe("bubble_sort");
    expect(r.newCode).toBe("maopao");
  });
});
