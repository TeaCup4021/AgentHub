...
| `archive/development/plans/` | 每日开发计划（归档） |
| `archive/development/summaries/` | 每日完成总结（归档） |
...
**最近更新（2026-06-10）** — 比赛提交文档整理

- **文档归档** — 开发过程产物（vibeCodingPlan/、vibeCodingSummary/、.vibe-coding/、BUGFIX-SUMMARY.md、AgentHub.txt）移入 `archive/`；根目录重复文件删除
- **README 重写** — 完整的比赛提交 README：功能亮点、架构图、技术栈、快速开始、项目结构
- **P2 状态修正** — 各文档中 P2 状态从矛盾的"全部完成/全部未做"统一为"6/8 完成（M20 会话分支、M21 task_hints 待开发）"
- **ADK 技术验证报告** → `docs/reference/`

**上次更新（2026-06-07）** — CLI Agent 内容重复 & Artifact 卡片缺失修复

- **CLI 流式文本重复修复** (`cli_runner.py:_parse_line`) — Claude Code 使用 `--include-partial-messages` 标志时同时输出增量 delta（`stream_event`）和完整消息（`assistant`），导致文本被累积两次，存库内容开头重复（如 "Now I'll create...Now I'll create..."）。修复：`_parse_line` 处理 `assistant` 事件时跳过 `text` 部分（文本已通过 `stream_event` delta 到达），保留 `thinking` 和 `tool_use` 部分（它们仅在 `assistant` 中出现）
- **CLI Agent Artifact 卡片缺失修复** (`cli_adapter.py:stream()`) — CLI adapter 的 `stream()` 被直接调用（不走 `_accumulate_stream_events` 包装器），故 artifact 检测从未执行：CLI 输出中的 `<artifact>` XML 标签在前端被 `stripArtifactTags()` 剥离，但没有生成 artifact 数据库记录，前端不渲染代码卡/部署状态卡。修复：`stream()` 方法持久化消息前调用 `detect_artifacts()`，yield SSE `artifact` 事件，持久化 artifact 到 DB；消息文本在持久化前剥离 artifact 标签
- **`strip_artifact_tags` 抽取到 `artifact_detector.py`** — 该函数原本定义在 `conversations.py`，CLI adapter 需要复用以避免循环导入。移入 `artifact_detector.py` 并设为公开函数，`conversations.py` 通过 `from app.services.artifact_detector import strip_artifact_tags` 导入并创建 `_strip_artifact_tags = strip_artifact_tags` 向后兼容别名
- **`_maybe_convert_pptx` 实现** (`artifact_detector.py`) — 上次修复已添加此函数（PPTX → PDF 转换），本次汇总记录
...
| ReAct 推理面板【P2】 | `ReActPanel` | 全局可拖拽推理面板（自动展开/图钉固定/位置记忆）+ 内嵌 ThinkingBlock |
| 产物工作台【P2】 | `ArtifactWorkbench` | 按会话聚合所有产物、筛选/搜索/全屏查看 |
| 首页落地页【P2】 | `WelcomePage` | 快捷操作卡片 + 最近对话 + 可用 Agent 网格 |
...
| PPT 内联浏览 | `DocumentCard` + pptx→pdf 转换 | ✅ 已完成（`_maybe_convert_pptx` 实现） |
| 会话分支【P2】 | 无前端组件 | BranchDialog 组件未实现，后端 branch API 待定 |
| @提及 task_hints【P2】 | `ChatInput.tsx` | 解析 @Agent+指令 为 task_hints 发送未实现 |
...
**P2 剩余（6/8 已完成）：**
1. **会话分支** — BranchDialog 组件 + 后端 `POST /conversations/{id}/branch` 端点
2. **@提及 task_hints 增强** — 解析 @Agent+指令，为 Orchestrator 提供预设子任务分配

**技术债：**
3. **API Key 加密存储** — Agent 敏感凭据加密/脱敏
4. **测试覆盖率补充** — 后端 pytest 和前端 vitest 关键路径覆盖
5. **Coordinator 分配优化** — 鼓励多 Agent 并行，减少同一 Agent 的串行 subtask 堆积
6. **消息编辑/删除 API** — 前端 MessageContextMenu 已预留入口
7. **Spec Manager DB 版** — 按 conversation 存储和解析规则
8. **CLI 会话复用** — 编排模式下多个 subtask 共享同一 CLI 会话
...
【场景】开始新功能开发时
【规则】必须先写 Plan（`docs/vibe-graph/plans/`），待确认后实现，完成后写 Summary（`archive/development/summaries/`）
